...
int[,] CreateMatrixMultiplication(int[,] firstMatrix, int[,] secondMatrix)
{
    ...
    for (int i = 0; i < thirdMatrix.GetLength(0); i++)
    {
        ...
        {
            sum = 0;
            for (int k = 0; k < thirdMatrix.GetLength(1) + 1; k++) // не совсем поняла почему +1, но только так правильно считает
            {
...
// не работает если все 4 параметра равны                             3 3 3 3
// не работает если строк А меньше столбцов В                         4 5 5 6
